...
# logging.basicConfig(filename="data/log-20190208.log", format="%(levelname)s - %(asctime)s - %(message)s", level=logging.INFO)
...
class Dataset(object):
    ...
    def _save(self):
        ...
        np.save("data/2-train.npy", np.array([X[: Aha], labels[: Aha]]))
        np.save("data/2-test.npy", np.array([X[Aha: ], labels[Aha: ]]))


    def _load(self):
        # disk or data
        if not Path("data/2-train.npy").exists():
            self._save()
        print("Loading ...")
        X_train, y_train = np.load("data/2-train.npy")
        X_test, y_test = np.load("data/2-test.npy")
        print("Finished!")
...
def train(model, train_set):
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        # target_names = ['non-sentiment', 'anger', "joy", "sadness", "fear"]
        target_names = ['non-', 'sentiment']
